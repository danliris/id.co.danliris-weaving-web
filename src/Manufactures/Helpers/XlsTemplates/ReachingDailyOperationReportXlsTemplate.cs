...
namespace Manufactures.Helpers.XlsTemplates
{
    public class ReachingDailyOperationReportXlsTemplate
    {
        public MemoryStream GenerateXls(List<DailyOperationMachineReachingDto> machineDtos, DateTime fromDate, DateTime toDate, string shift, string mcNo)
        {
            ...
            reportDataTable.Columns.Add(new DataColumn() { ColumnName = "EFF", DataType = typeof(string) });
            reportDataTable.Columns.Add(new DataColumn() { ColumnName = "Jumlah Beam", DataType = typeof(double) });
            int idx = 1;
            double sumBeam = 0;
            foreach (var item in machineDtos)
            {
                var eff =Math.Round(Convert.ToDouble(item.Efficiency) * 100, 2);
                var efficiency = eff.ToString() + "%";
                var dateFormat = "dd MMM yyyy";
                var date = item.Periode.ToString(dateFormat);
                reportDataTable.Rows.Add(idx, date, item.Operator, efficiency, item.BeamNo);
            ...
            using (var package = new ExcelPackage())
            {
                ...
                worksheet.Cells["A" + 1 + ":E" + 1 + ""].Merge = true;
                worksheet.Cells["A" + 2 + ":E" + 2 + ""].Merge = true;
                worksheet.Cells["A" + 3 + ":E" + 3 + ""].Merge = true;
                worksheet.Cells["A" + 4 + ":E" + 4 + ""].Merge = true;
                worksheet.Cells["A" + 5 + ":E" + 5 + ""].Merge = true;
                worksheet.Cells["A" + 1 + ":E" + 7 + ""].Style.Font.Bold = true;
                ...
                worksheet.Cells["E" + (idx + 7)].Value = sumBeam;
                ...
                worksheet.Cells["A" + (idx + 7) + ":E" + (idx + 7) + ""].Style.Font.Bold = true;

                worksheet.Cells["A" + 6 + ":E" + (idx + 7) + ""].Style.Border.Bottom.Style = ExcelBorderStyle.Thin;
                worksheet.Cells["A" + 6 + ":E" + (idx + 7) + ""].Style.Border.Top.Style = ExcelBorderStyle.Thin;
                worksheet.Cells["A" + 6 + ":E" + (idx + 7) + ""].Style.Border.Left.Style = ExcelBorderStyle.Thin;
                worksheet.Cells["A" + 6 + ":E" + (idx + 7) + ""].Style.Border.Right.Style = ExcelBorderStyle.Thin;

                worksheet.Cells["A" + 1 + ":E" + (idx + 7) + ""].AutoFitColumns();
